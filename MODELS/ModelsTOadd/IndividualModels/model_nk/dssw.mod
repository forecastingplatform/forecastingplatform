...
L_adj,              uniform_pdf,     662,    10, 650, 670;
//D_adj,              uniform_pdf,     0.3,    0.1, 0, 1;
...
estimation(mode_compute=6,datafile=data,first_obs=1,nobs=152,plot_priors=0,nodiagnostic,mh_replic=0,mh_nblocks=1,mh_jscale=0.4,mh_drop=0.2);
...
//load results;
//stoch_simul(order=1,irf=0) dlnY, dlnC, dlnI, dlnw, lnL, dlnP, FedFunds;

//for jj=1:size(M_.endo_names,1);eval([deblank(M_.endo_names(jj,:)) '=' num2str(oo_.steady_state(jj),11) ';']);end;
