...
//Based on replication files of  "Forecasting with DSGE models with Financial Frictions" - by Marcin Kolasa and Micha Rubaszek
//Adapted to the forecast platform: Matyas Farkas
//close all;

var lambda R pic c w_tilda w phi_w psi_w L k k_bar i Q R_e r_k u 
    mc p_tilda phi_f psi_f y Delta
    z lb lphi lmu llambda_f lg lA llamda_fma 
	hours_obs rff_q_obs pgdp_q_obs xgdp_q_obs pcer_q_obs fpi_q_obs wage_obs; 
...
llambda_f=(1-rho_lambda_f)*log(lambda_f)+rho_lambda_f*llambda_f(-1)+ llamda_fma - cmap*llamda_fma(-1); // Orignal SW07 model had:	      spinf = crhopinf*spinf(-1) + epinfma - cmap*epinfma(-1);	          epinfma=epinf;
llamda_fma = eps_lambda_f/100;
...
alpha,              0.33; 	//0.2647;//0.154;
zeta_p,             0.6;  	// 0.5567;//0.77;
iota_p,             0.5;  	//0.1332;//0.06;
s_bis,              0.4;  	//0.3014;//0.5;
h,                  0.7;	//0.7427;//0.77;
a_bis,              0.2;	//0.2492;//0.24;
nu_l,               2;		//2.1929;//2.01;
zeta_w,             0.6;	//0.2223;//0.25;
iota_w,             0.5;	//0.0735;//0.12;
r_star,             2;		//1.7943;//1.01;
psi_1,              1.5;	//2.0128;//1.69;
psi_2,              0.2;	//0.1185;//0.03;
rho_R,              0.5;	//0.5849;//0.74;
...
//nu,                 0.975;	//0.9779;
//sigma,              0.30;  	//0.2847;
rho_z,              0.09;	//0.0998;//0.07;
rho_phi,            0.9;		//0.9802;//0.98;
rho_lambda_f,       0.9;		//0.8715;//0.91;
rho_mu,             0.9;		//0.8434;//0.80;
rho_b,              0.9;		//0.2005;//0.22;
rho_g,              0.9;		//0.9274;//0.96;
//rho_A,              0.1;
//rho_sigma,          0.2;
//rho_nu,             0.2;
stderr eps_z,       0.7519;
...
*/
//stoch_simul(order=1,irf=16) xgdp_q_obs hours_obs wage_obs pgdp_q_obs rff_q_obs pcer_q_obs fpi_q_obs;
