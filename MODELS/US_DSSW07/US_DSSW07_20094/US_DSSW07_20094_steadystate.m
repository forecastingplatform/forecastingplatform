...
for jj=1:size(M_.param_names)-nparam_ss;
    eval([deblank(M_.param_names(jj,:)),'=',num2str(M_.params(jj,:)),';'])
end
...
%% K was originally computed using the equation system
%F2=0.1; f_k= @(k)solve_k(k,g_star,alpha,gamma,beta,delta,h,chi,lambda_w,phi,nu_l,r_k,w,F2,R_e,pic_star);
%k       = fzero(f_k,25);
%% A new function to compute k. - Matyas
f_k = @(k) (k-solve_k(k,alpha,r_k,w,h,phi,nu_l,lambda_w,gamma_star,r_star,delta,gg_star));
optimset('TolX',1e-14);
k = fzero(f_k,25);
...
function k1 = solve_k(k,alpha,r_k,w,h,phi,nu_l,lambda_w,gamma_star,r_star,delta,gg_star)
