...
var   hours_obs rff_q_obs pgdp_q_obs xgdp_q_obs pcer_q_obs fpi_q_obs wage_obs  
...
//the next parameters are declared in the model section: 
//clandap cpie cgamma cbeta cbetabar cr crk cw cikbar cik clk cky ciy ccy crkky cwhlc cwly conster
...
model(linear); 
//#usmodel_stst;
//the following lines were originally in usmodel_stst, but this does not work with Dynare 4
#cpie=1+constepinf/100;
#cgamma=1+ctrend/100 ;
#cbeta=1/(1+constebeta/100);     % constebeta is 100(beta^(-1)-1) which is estimated
#clandap=cfc;
#cbetabar=cbeta*cgamma^(-csigma);
#cr=cpie/(cbeta*cgamma^(-csigma));
#crk=(cbeta^(-1))*(cgamma^csigma) - (1-ctou);
...
xgdp_q_obs=y-y(-1)+ctrend;
pcer_q_obs=c-c(-1)+ctrend;
fpi_q_obs=inve-inve(-1)+ctrend;
wage_obs=w-w(-1)+ctrend;
pgdp_q_obs = 1*(pinf) + constepinf;
rff_q_obs =    1*(r) + conster;
hours_obs = scalehours*lab + constelab;
...
//varobs xgdp_q_obs pgdp_q_obs fpi_q_obs hours_obs pcer_q_obs wage_obs rff_q_obs;