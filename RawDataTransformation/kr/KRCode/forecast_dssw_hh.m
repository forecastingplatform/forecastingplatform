...
for i=90:110
    for j=1:4
       ...
       load(['hh/',num2str(i),'q',num2str(j),'/results.mat']);
       ...
       % reads MH settings
       load(['hh/',num2str(i),'q',num2str(j),'/dssw_hh_mh_history.mat']);
       ...
       draw_.fname         = ['hh/',num2str(i),'q',num2str(j),'/dssw_hh'];
       ...
       f.dlnP     = NaN(h,n*m*nrep); f.Spreadi = NaN(h,n*m*nrep); f.dlnDin = NaN(h,n*m*nrep);
       ...
       for ChainNumber = 1:n
            for DrawNumber  = 1:m
               ...
               f.dlnP(:, place)      = fdraws.dlnP;     f.Spreadi(:, place) = fdraws.Spreadi;
               f.dlnDin(:, place)    = fdraws.dlnDin;
       ...
       save(['hh/',num2str(i),'q',num2str(j),'/forecasts.mat'], 'f');  
    end
end
